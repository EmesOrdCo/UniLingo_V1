...
/**
 * Runner script for Italian lesson script translation
 * 
 * This script:
 * 1. Runs the SQL migration to add Italian lesson script columns
 * 2. Populates the Italian lesson script translations using OpenAI
 */
...
async function main() {
  log('🇮🇹 Italian Lesson Script Translation Setup', 'bright');
  log('==========================================', 'bright');
  
  try {
    // Check if migration file exists
    const migrationFile = path.join(projectRoot, 'database', 'migrations', 'add_italian_lesson_script_columns.sql');
    if (!fs.existsSync(migrationFile)) {
      throw new Error('Migration file not found: add_italian_lesson_script_columns.sql');
    }
    ...
    const translationScript = path.join(projectRoot, 'scripts', 'utilities', 'populate_italian_lesson_scripts.js');
    if (!fs.existsSync(translationScript)) {
      throw new Error('Translation script not found: populate_italian_lesson_scripts.js');
    }
    ...
    await execCommand(
      ...
      'Running full Italian lesson script translation'
    );
    
    log('\n🎉 Italian lesson script translation setup completed successfully!', 'green');
    log('\nSummary:', 'bright');
    log('✅ SQL migration applied (manually)', 'green');
    log('✅ Italian lesson script translations populated', 'green');
    log('✅ All lesson scripts now have Italian translations', 'green');
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  log('Italian Lesson Script Translation Setup', 'bright');
  log('Usage:');
  log('  node run-italian-lesson-scripts.js');
  log('\nThis script will rename lesson_script columns and populate Italian translations.');
  log('\nPrerequisites:');
