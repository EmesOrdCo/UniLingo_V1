...
/**
 * Runner script for Cantonese lesson script translation
 * 
 * This script:
 * 1. Runs the SQL migration to add Cantonese lesson script column
 * 2. Populates the Cantonese lesson script translations using OpenAI
 */
...
async function main() {
  log('🇭🇰 Cantonese Lesson Script Translation Setup', 'bright');
  log('============================================', 'bright');
  
  try {
    // Check if migration file exists
    const migrationFile = path.join(projectRoot, 'database', 'migrations', 'add_cantonese_lesson_script_columns.sql');
    if (!fs.existsSync(migrationFile)) {
      throw new Error('Migration file not found: add_cantonese_lesson_script_columns.sql');
    }
    ...
    const translationScript = path.join(projectRoot, 'scripts', 'utilities', 'populate_cantonese_lesson_scripts.js');
    if (!fs.existsSync(translationScript)) {
      throw new Error('Translation script not found: populate_cantonese_lesson_scripts.js');
    }
    ...
    await execCommand(
      ...
      'Running full Cantonese lesson script translation'
    );
    
    log('\n🎉 Cantonese lesson script translation setup completed successfully!', 'green');
    log('\nSummary:', 'bright');
    log('✅ SQL migration applied (manually)', 'green');
    log('✅ Cantonese lesson script translations populated', 'green');
    log('✅ All lesson scripts now have Cantonese translations', 'green');
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  log('Cantonese Lesson Script Translation Setup', 'bright');
  log('Usage:');
  log('  node run-cantonese-lesson-scripts.js');
  log('\nThis script will add Cantonese lesson script columns and populate translations.');
  log('\nPrerequisites:');
