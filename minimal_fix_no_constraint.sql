-- Minimal fix: Change the link from subject_id to subject_name
-- This version doesn't add a foreign key constraint since subject isn't unique

-- Step 1: Check current structure
SELECT 'Current lesson_scripts table structure:' as info;
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
WHERE table_name = 'lesson_scripts' 
ORDER BY ordinal_position;

-- Step 2: Drop the existing lesson_scripts table to recreate with correct link
DROP TABLE IF EXISTS lesson_scripts CASCADE;

-- Step 3: Recreate lesson_scripts table with subject_name (no foreign key constraint)
CREATE TABLE lesson_scripts (
  id SERIAL PRIMARY KEY,
  subject_name TEXT NOT NULL, -- This links to subject_words.subject (logical relationship)
  english_script_writing TEXT,
  english_script_roleplay TEXT,
  french_script_writing TEXT,
  french_script_roleplay TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Step 4: Add unique constraint on subject_name to ensure one script per subject
ALTER TABLE lesson_scripts 
ADD CONSTRAINT unique_subject_name UNIQUE (subject_name);

-- Step 5: Add indexes for better performance
CREATE INDEX idx_lesson_scripts_subject_name ON lesson_scripts(subject_name);

-- Step 6: Add comments
COMMENT ON TABLE lesson_scripts IS 'Scripts for lessons - one per subject';
COMMENT ON COLUMN lesson_scripts.subject_name IS 'Links to subject_words.subject - one script per subject (logical relationship)';

-- Step 7: Show the corrected structure
SELECT 'Final lesson_scripts table structure:' as info;
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
WHERE table_name = 'lesson_scripts' 
ORDER BY ordinal_position;

-- Step 8: Show the relationship
SELECT 'Logical Relationship: lesson_scripts.subject_name matches subject_words.subject' as relationship_info;
SELECT 'Constraint: UNIQUE on subject_name ensures one script per subject' as constraint_info;

-- Step 9: Example query to verify the relationship works
SELECT 'Example query to get subject words and script:' as info;
SELECT '
SELECT 
    sw.word_phrase,
    sw.french_keyword,
    ls.english_script_writing
FROM subject_words sw
LEFT JOIN lesson_scripts ls ON sw.subject = ls.subject_name
WHERE sw.subject = ''cooking'';
' as example_query;
