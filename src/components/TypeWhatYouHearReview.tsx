...
interface TypeWhatYouHearReviewProps {
  visible: boolean;
  onClose: () => void;
  onPlayAgain: () => void;
  gameResults: {
...
const TypeWhatYouHearReview: React.FC<TypeWhatYouHearReviewProps> = ({
  visible,
  onClose,
  onPlayAgain,
  gameResults,
  ...
  const getScoreMessage = (score: number) => {
    if (score >= 90) return 'Listening Master! 👂';
    if (score >= 80) return 'Great listening! 🎧';
    if (score >= 70) return 'Good job! 👍';
    if (score >= 60) return 'Not bad! 💪';
    return 'Keep practicing! 📚';
  };

  const getDifficultyLevel = (playbackSpeed: number) => {
    if (playbackSpeed <= 0.8) return { color: '#10b981', text: 'Easy' };
    if (playbackSpeed <= 1.0) return { color: '#f59e0b', text: 'Normal' };
    if (playbackSpeed <= 1.2) return { color: '#ef4444', text: 'Hard' };
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.reviewSection}>
            ...
            {words.map((word, index) => {
              const difficulty = getDifficultyLevel(word.playbackSpeed);
              return (
...
export default TypeWhatYouHearReview;
