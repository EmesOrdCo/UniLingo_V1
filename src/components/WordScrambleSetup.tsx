...
export interface WordScrambleSetupOptions {
  wordCount: number;
...
interface WordScrambleSetupProps {
  visible: boolean;
  onClose: () => void;
  onStartGame: (options: WordScrambleSetupOptions) => void;
  availableCards: number;
}

const WordScrambleSetup: React.FC<WordScrambleSetupProps> = ({
  visible,
  onClose,
  onStartGame,
  availableCards,
  ...
  // Reset word count if it exceeds available cards after filtered count updates
  useEffect(() => {
    if (filteredCardCount > 0 && wordCount > filteredCardCount) {
      // Find the largest valid option
      const validOptions = [5, 10, 15, 20].filter(count => count <= filteredCardCount);
      if (validOptions.length > 0) {
        setWordCount(Math.max(...validOptions));
      }
    }
  }, [filteredCardCount]);
  ...
  const handleStartGame = () => {
    ...
    const options: WordScrambleSetupOptions = {
      wordCount,
    ...
    >
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>{t('gameSetup.title.wordScramble')}</Text>
        ...
        <View style={styles.content}>
              ...
              {wordCountOptions.map((option) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optionCard,
                      wordCount === option.value && styles.optionCardSelected,
                  ...
                  >
                    <Text style={[
                      styles.optionLabel,
                      wordCount === option.value && styles.optionLabelSelected,
          ...
          <TouchableOpacity style={styles.startButton} onPress={handleStartGame}>
            <Text style={styles.startButtonText}>{t('gameSetup.buttons.startWordScramble')}</Text>
...
export default WordScrambleSetup;
